  ...
  return (
    <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 min-h-screen pt-20 pb-16">
      <motion.div
        ...
        className="mb-4 md:mb-8 flex items-center justify-center"
      >
      ...
      <motion.span
        ...
        className="font-poppins uppercase tracking-[0.35em] text-[10px] md:text-sm text-primary mb-4 md:mb-6 font-medium"
      >
      ...
      <motion.h1
        ...
        className="font-cinzel text-4xl md:text-7xl lg:text-8xl text-accent font-medium my-2 md:my-4 tracking-wide leading-tight max-w-4xl"
      >
        {weddingData.groom.shortName}
        <span className="block font-cormorant italic text-primary text-3xl md:text-6xl my-1.5 md:my-3 font-light">
          &
        </span>
      ...
      <motion.p
        ...
        className="font-cormorant text-xl md:text-3xl text-text/90 font-light mt-4 md:mt-6 mb-8 md:mb-12 tracking-wider"
      >
        {formattedDate}
      </motion.p>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="grid grid-cols-4 gap-3 md:gap-6 max-w-xl mx-auto mb-8 md:mb-16 px-2 w-full"
      >
        {timeUnits.map((unit) => (
          <motion.div key={unit.label} variants={VARIANTS.scaleUp(reducedMotion)}>
            <div className="flex flex-col items-center justify-center py-4 px-2 md:py-6 md:px-5 rounded-2xl border border-primary/15 bg-primary/[0.03] backdrop-blur-md text-center shadow-[0_8px_32px_rgba(199,169,127,0.06)] hover:border-primary/30 transition-all duration-500">
              <span className="font-cinzel text-2xl md:text-5xl text-accent font-light mb-1.5 tracking-tight">
                {String(unit.value).padStart(2, '0')}
              </span>
              <span className="font-poppins uppercase text-[8px] md:text-xs tracking-[0.25em] text-primary/80 font-medium">
                {unit.label}
              </span>
            </div>
          </motion.div>
